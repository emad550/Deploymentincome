...
@application.route('/predict',methods=['GET','POST']) # route to show the predictions in a web UI
@cross_origin()
def index():
    if request.method == 'POST':
        try :
            ...
            if (relationship == 'Not-in-family'):
                ...
                relationship = 2
            filename = 'Randomforest_le.pkl'
            loaded_model = pickle.load(open(filename, 'rb')) # loading the model file from the storage
            # predictions using the loaded model file
            prediction=loaded_model.predict([[year,workclass,fnlwgt,education,educationnum,maritalstatus,occupation,relationship,2,3,4,1,2,7]])
